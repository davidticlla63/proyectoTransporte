...
@Named(value = "reporteEstadoResultadoController")
@ConversationScoped
public class ReporteEstadoResultadoController implements Serializable {
	...
	private String urlEstadoResultado;
	...
	public void procesarReporte(){
		...
		for(PlanCuenta pc : listPlanCuenta){
			...
			if(tipo.equals("INGRESO") || tipo.equals("EGRESO")  || tipo.equals("GASTOS")  ||tipo.equals("COSTOS") ){
	...
	public String loadURL(){
		try{
			...
			String urlPDFreporte = urlPath+"ReporteEstadoResultado?pFechaInicio="+Fechas.deDateAString(fechaInicial)+"&pFechaFin="+Fechas.deDateAString(fechaFinal)+"&pIdGestion="+gestionLogin.getId()+"&pIdEmpresa="+empresaLogin.getId()+"&pUsuario="+nombreUsuario;
	...
	public void actualizarForm(){
		seleccionado = false;
		urlEstadoResultado = loadURL();
		log.info("cargando......");
		RequestContext context = RequestContext.getCurrentInstance();
		context.execute("PF('dlgVistaPreviaEstadoResultado').show();");
	...
	public String getUrlEstadoResultado() {
		return urlEstadoResultado;
	}

	public void setUrlEstadoResultado(String urlEstadoResultado) {
		this.urlEstadoResultado = urlEstadoResultado;
	}
