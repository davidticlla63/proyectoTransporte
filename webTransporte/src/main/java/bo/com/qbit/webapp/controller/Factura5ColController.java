...
@Named(value = "factura5ColController")
@ConversationScoped
public class Factura5ColController implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 2432204623331827974L;
	...
	Logger log = Logger.getLogger(Factura5ColController.class);
	...
	Event<String> pushEventFactura5Col;
	...
	private @Inject UnidadMedidaRepository unidadMedidadRepository;
	...
	private void loadValuesDefaul() {
		...
		listUnidadMedida = unidadMedidadRepository
		...
		modificar = false;
		log.info("inicio de loadValuesDefaul");
	}
	...
	public List<Producto> completeText(String query) {
		...
		for (Producto i : listProducto) {
			if ((i.getNombre().toUpperCase().startsWith(upperQuery.toUpperCase()))
	...
	private void registrarUnidadMedida() {
		try {
			if (!existeUnidadMedida()) {
				...
				unidadMedida = unidadMedidaRegistration.create(unidadMedida);
			}
		} catch (Exception e) {
	...
	private boolean existeUnidadMedida() {
		try {
			...
			List<UnidadMedida> list = unidadMedidadRepository
					.findAllActivasByEmpresa(empresaLogin, medida.toUpperCase());
			if (list.size() > 0) {
	...
	public void dialogClose() {
		...
		String navigateString = request.getContextPath()
				+ "/pages/formulario/facturacion5Col.xhtml";
	...
	public void agregarProducto() {
		try {
			...
			detalleFactura.setUnidadMedida(medida);
	...
	public List<String> completeTextUnidadMedida(String query) {
		log.info("ingreso completeTextUnidadMedida "+listUnidadMedida.size());
		List<String> results = new ArrayList<String>();
		if (query.trim().length()>0) {
			for (UnidadMedida i : listUnidadMedida) {
				if (i.getSigla().startsWith(query.toUpperCase())) {
					medida = i.getSigla();
					log.info("Agregado .." + i.getSigla());
					results.add(i.getSigla());			
				}
			}
		}else{
			for (UnidadMedida i : listUnidadMedida) {
	...
	public void onItemSelectUnidad(SelectEvent event) {
		String unidades = event.getObject().toString();
		log.info("ingreso onItemSelectUnidadMedida " + unidades);
		for (UnidadMedida unidadMedida : listUnidadMedida) {
			if (unidadMedida.getSigla().equals(unidades)) {
				medida = unidadMedida.getSigla();
				log.info("Unidad Medida encontrado .." + unidades);
			}
		}
	}
