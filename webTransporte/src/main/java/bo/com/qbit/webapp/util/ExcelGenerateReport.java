...
import bo.com.qbit.webapp.controller.ExportacionController;
...
public class ExcelGenerateReport {
	...
	private int insertDetailInfo(XSSFSheet sheet, int index) {
		...
		log.info("listFactura : "+listFactura.size());
		...
		for (int i = 0; i < listFactura.size(); i++) {
			log.info(listFactura.get(i).toString());
		...
		log.info("FINALIZO EL FOR" +row.getRowNum());
