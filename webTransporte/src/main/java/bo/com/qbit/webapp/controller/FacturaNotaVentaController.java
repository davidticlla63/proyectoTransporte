...
import bo.com.qbit.webapp.data.NotaVentaRepository;
	...
	public void revisarFacturas(){
		...
		for (Factura factura : listFactura) {
			dosificacion = dosificacionRepository.findActivaBySucursal(factura
					.getSucursal());
			list+=factura.getNumeroFactura()+"|"+Time.convertSimpleDateToString(factura.getFechaFactura())+"|"+factura.getNumeroAutorizacion()+"|"+factura.getNitCi()+"|"+factura.getNombreFactura()+"|"+factura.getTotalPagar()+"|"+control7.obtenerCodigoControl2(factura.getFechaFactura(),factura.getNumeroFactura(), dosificacion, factura.getTotalPagar(), factura.getNitCi())+"\n";
	...
	public void armarUrlFactura5Colunmas() {
		try {
			...
			formatoHoja = formatoHojaRepository.findActivosByEmpresa(empresaLogin,sucursalLogin)
					.get(0);
			if (formatoHoja.getNombre().equals("COMPLETO")) {
				url = urlPath + "ReportFactura5Col?pIdFactura="
						+ selectedFactura.getId() + "&pEmpresa="
						+ empresaLogin.getRazonSocial() + "&pCiudad="
						+ empresaLogin.getCiudad() + "&pPais=BOLIVIA&pLogo="
						+ urlLogo + "&pNit=" + empresaLogin.getNit() + "&pQr="
...
/*		resetearFitrosTabla("formTableFactura:dataTableFactura");*/
	...
	public void anularFactura() {
		try {
			...
			NotaVenta notaVenta= notaVentaRepository.findAllActivasByEmpresaSucursalForFactura(empresaLogin, sucursalLogin,selectedFactura);
			if (notaVenta!=null) {
